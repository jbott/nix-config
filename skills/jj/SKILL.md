...
| `jj tug` | Advance the nearest bookmark to the latest non-empty commit in `::@`. **Only when a `john/*` bookmark already exists** — with none in `::@` the nearest is `main`, and tug drags your local `main` onto your work. Run `jj bookmark create john/<name>` first. | After committing on a stack you want a bookmark to follow |
...
| `jj w ls` | List workspaces as `<name><TAB><path>`. `--stale` lists only those whose directory is gone from disk. | Seeing what workspaces exist and where |
| `jj w new <name>` (`jj w add <name>`) | Create (or reuse) the workspace for `<name>` at `<parent-of-repo>/.worktrees/<repo>/<name>`, print its path. Creates no bookmark. | Adding a workspace |
| `jj w rm <ws>...` | Forget each workspace and `rm -rf` its directory. `--stale` (no names) forgets every tracked workspace whose directory is already gone. | Cleaning up a workspace |
...
# Local bookmark already EXISTS (e.g. you ran `jj bookmark create`) but was
# never pushed — push it by name (--named would fail, it already exists):
...
`jj git push --named` **creates the bookmark as part of the push** — do **not**
run `jj bookmark create` first, or it fails with `Bookmark already exists`. Point
it at `@-` after a `jj commit` (the work is at `@-`, `@` is the fresh empty
commit). If the local bookmark already exists, push it by name with
`--bookmark <name>` instead.

Why a bare `jj push` won't create it: `auto-track-bookmarks` only auto-tracks
...
That is the signal to push with `--named` (above) — **not** to add `--allow-new`
(removed in this jj; it errors `unexpected argument '--allow-new'`) or `--force`
(jj force-with-leases automatically; on rejection `jj git fetch` then re-push).
